...
class BrowserLauncher:
    ...
    def _browser_args(self, label: str) -> list[str]:
        return [
            "--new-window",
                ...
                wrapper_path.write_text(
                    "\n".join(
                        [
                            ...
                            f"start \"NetVisor {label.title()}\" \"{executable}\" {browser_args} %*",
                        ]
                    ),
                    encoding="utf-8",
                )
    ...
    def launch(self, process_name: str, url: str | None = None) -> bool:
        ...
        args = [str(executable), *self._browser_args(label)]
        if url:
            args.append(url)
        try:
